...
class autentic:
    ...
    #verifica se já existe um chat desse usuario
    def checkuserchat(senderusername, chats):
        for check in chats:
            if senderusername == check['username']:
                return check
        return 0
    
    #verifica se já existe um chat desse grupo
    def checkcommunitychat(sendername, chats):
        for check in chats:
            if sendername == check["name"]:
                return 1
        return 0
    
    #verifica e retorna o nível de acesso do usuario
