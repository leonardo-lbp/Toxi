...
class UserProfileScreen(BaseScreen):
            ...
            [
                ...
                sg.Column(
                    [
                        ...
                        [sg.Text('', size=(5,0), background_color=cor_roxo_escuro), sg.Text('Comunidades', font=('Helvetica', 15), background_color=cor_roxo_escuro, key='com', enable_events=True)],
                        [sg.Text('', size=(0, 20),background_color=cor_roxo_escuro)]
                    ],
                    element_justification='c',
                    background_color=cor_roxo_escuro
    ...
    def events(self, event, value):
        ...
        if event == 'search':
            ...
            window_search = sg.Window('Buscar', layout_search)

            while True:
                event_search, values_search = window_search.read()
                if event_search == sg.WIN_CLOSED:
                    break
                elif event_search == 'Search':
                    search_term = values_search['search_term']
                    sg.popup(f"Buscando por: {search_term}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        inciar.run()
        profile.set_data()
    except exception_erros.InvalidDataError:
        sg.popup_error("Dados não encontrados!")